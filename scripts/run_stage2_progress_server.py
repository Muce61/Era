...
from era100x.research.stage_2.acceptance.evidence_gate.cli import (
    status_payload as evidence_gate_status,
)
...
def _stage2_v16_projection(stage2_root: Path) -> dict[str, Any]:
    """Project Plan v1.6/T19 using only policy and append-only evidence."""
    ...
    policy = load_evidence_gate_policy(
        REPOSITORY_ROOT / "configs/governance/stage2_active_policy_v5.json",
        repository_root=REPOSITORY_ROOT,
    )
    status = evidence_gate_status(policy, REPOSITORY_ROOT)
...
def _stage2_v110_projection(stage2_root: Path) -> dict[str, Any]:
    ...
    for index, name in enumerate(S2P110_TASK_ORDER):
        if index < processed_tasks:
            ...
            total = int(metrics.get("total_units") or 1)
            task_status = (
                "BLOCKED"
                if current_run.get("last_event") in {"TASK_INTERRUPTED", "RUN_FAILED"}
                else "IN_PROGRESS"
            )
            subphase = str(current_run.get("subphase") or current_run.get("last_event"))
        else:
            processed, total, task_status = 0, 1, "NOT_STARTED"
            subphase = (
                "PREPARE_INPUTS_LOCK_PENDING"
                if index == 0 and inputs_lock_count == 0
                else "UPSTREAM_TASK_COMPLETION_PENDING"
            )
        phases.append(
            {
                "name": name,
...
def _stage2_conditional_baseline_projection(stage2_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "governance_terminal_status": "STOPPED_FAILED_UNPUBLISHED",
        ...
        "is_direct_t20_dependency": False,
        "resume_allowed": False,
        "status": status,
        "reason_code": reason,
    ...
    def do_GET(self) -> None:  # noqa: N802 - BaseHTTPRequestHandler API
        ...
        if path == "/":
            ...
            try:
                ...
                self._reply_json(HTTPStatus.SERVICE_UNAVAILABLE, {"error": str(exc)})
        elif path == "/api/v16/status":
            try:
                self._reply_json(
                    HTTPStatus.OK,
                    {"stage2_plan_v16": _stage2_v16_projection(self.server.stage2_root)},
                )
            except (OSError, ValueError) as exc:
                self._reply_json(HTTPStatus.SERVICE_UNAVAILABLE, {"error": str(exc)})
