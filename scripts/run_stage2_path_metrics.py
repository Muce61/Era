#!/usr/bin/env python3
"""Approved S2-T12 v1.3 historical path metrics CLI."""
...
def main() -> int:
    ...
    if args.mode == "preflight":
        payload, path = create_preflight_manifest(code_commit=current_code_commit())
        result = {
            "status": "PASS",
            "mode": "preflight",
        ...
        resumed = resume_run(run_root)
        result = {"status": "PASS", "mode": "resume", "run_root": str(resumed)}
    else:
        if not args.run_id:
