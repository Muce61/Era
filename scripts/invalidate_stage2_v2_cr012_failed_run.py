#!/usr/bin/env python3
"""Append-only invalidation for the terminal CR-2026-012 predecessor Run B."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if subprocess.check_output(["git", "rev-parse", "HEAD"], text=True).strip() != args.code_commit:
    ...
    if len(monthly) != EXPECTED_MONTHLY or len(packed) != EXPECTED_PACKED or published:
    ...
    payload: dict[str, Any] = {
        ...
        "retained_staging_file_count": len(staging_files),
        "published_files": 0,
    ...
    path = failed_root / "reports" / "invalidation-cr-2026-012.json"
