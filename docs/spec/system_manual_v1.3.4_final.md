...
document_version: V1.3.4
source_document: system_manual_v1.3.4_final.docx
supersedes:
  - V1.2
  - V1.3
  - V1.3.1
...
Research & Engineering Manual · V1.3.4
...
V1.3.4 以 V1.3.3 为唯一正文基线，仅处理五项最终定稿阻塞：区分 LOCAL_SUBMISSION 与 VENUE_OBSERVED Bootstrap；补齐两个原子事务的必填字段；修复不变量编号与旧权益字段；冻结 Stage B/Stage C 单一职责；压缩明显空白分页。其他已通过的项目目标、证据边界、灾难保护架构、研究路线和阶段门槛保持不变。
...
| U-010 | +20%净ROE激活和阶梯参数 | RESEARCH | 路径依赖回放和锁定历史重放 | 阶段3-4 | 持仓规则 |
| U-011 | 5/8分钟及15/25分钟退出 | RESEARCH | Time-to-Activation/Time-since-MFE统计 | 阶段2-3 | 持仓规则 |
