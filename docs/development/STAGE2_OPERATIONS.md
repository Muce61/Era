...
authority is `configs/governance/current_development_state.json`, which points to Policy v8 /
Plan v1.9. It permits read-only work, append-only source audit supplements and isolated seven-day
...
Plan v1.9 uses `scripts/run_stage2_v19.py` with four commands:
`status / prepare / run / resume`. `prepare` creates the single full-period inputs lock and prints
the exact commit, Policy, preregistration, contract-bundle and input-lock Hashes. After one human
