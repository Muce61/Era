...
| OQ-S2-004 | 2026-07-16审批引用的T1/T3/T4精确时间组合、三个预注册时期、条件随机匹配bin/固定放宽层级和主失败线在仓库及全部本地历史中均无定义；其精确值是什么？ | OPEN | Stage 2 preregistration completion | S2-T19 and Stage 2 Group 1 execution | 需Muce补充精确tuple、UTC日期边界、匹配bin/放宽层级和数值失败线。Codex不得从数据或手册未冻结内容推断；补齐前S2-T19必须fail-closed。 |
