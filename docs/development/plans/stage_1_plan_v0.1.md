...
建立“历史数据基础”所需的可审计能力，并只在本阶段证据边界内形成可验收产物。
...
- 系统不变量：以 `traceability/rules.yaml` 中 planned_stage 包含 S1 的 INV 条目为准
- Reason Code：以附录 I 和机器追踪中 planned_stage 包含 S1 的条目为准
...
满足手册第 29 节及附录 L 的 Stage 1 门槛；全部 Task 有审查和真实验证证据；追踪无未解释缺口；人工验收。
...
仅引用 `OPEN_QUESTIONS.md` 中解决阶段包含 Stage 1 的问题；不得在本计划中自行决策。
...
`stage-1-vX.Y-passed`（仅 PASSED 且人工批准后创建；本次不得创建）。
