...
交付并验证“状态与 Reason Code 枚举”这一单一能力，使其可独立提交、审查和回滚。
...
- 手册章节：§20、§22、§41、§43-44、附录G/I
- rule_id：STATE-BREAKER-PERSIST、CLOSE-THREE-STAGE、CLOSURE-STAGE-SINGLE-RESPONSIBILITY（仅枚举/静态约束）
- 数据契约：核心状态、reconcile_phase、订单/Algo/leg状态、ExitDecision、Reason Code、Incident等级
...
建立枚举、Reason Code注册表和未列组合默认RECONCILING的静态声明；不实现状态机或任何交易所写动作。
...
`src/<root_package>/foundation/state/`、`tests/foundation/state/`
...
第5节能力的最小产物、对应测试、`docs/development/validations/stage_0/S0-T09.md`和精确追踪更新；不得捆绑下一Task。
...
附录G/I值完整唯一；DEPRECATED状态不可重新使用；Reason有唯一分类/默认动作；Stage B/C职责可静态校验。

## 13. 验收标准

状态和Reason覆盖报告无缺口；T-INV-ID、T-STAGE中的静态部分通过；行为部分明确留待Stage 6。

## 14. 必须运行的命令

```bash
python3.12 -m pytest tests/foundation/state -q
```
```bash
python3.12 scripts/check_reason_code_coverage.py
```
```bash
...
- 2026-07-12：状态、closure phase、ExitDecision、事故等级和Reason Code覆盖通过，验收PASS。
