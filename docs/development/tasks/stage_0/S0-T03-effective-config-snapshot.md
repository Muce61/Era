...
交付并验证“配置层级与有效配置快照”这一单一能力，使其可独立提交、审查和回滚。
...
- 手册章节：§5 配置优先级、附录B
- rule_id：STATE-BREAKER-PERSIST（配置哈希输入）；FROZEN/BASELINE/RESEARCH 状态不得漂移
- 数据契约：EffectiveConfigSnapshot、effective_config.json、config_hash
...
实现配置优先级、模式限制、不可变有效快照和确定性哈希；只提供规格中的默认模板及状态，不优化参数。
...
`src/<root_package>/foundation/config/`、`configs/`、`tests/foundation/config/`
...
第5节能力的最小产物、对应测试、`docs/development/validations/stage_0/S0-T03.md`和精确追踪更新；不得捆绑下一Task。
...
相同输入哈希一致；优先级确定；live/compound拒绝CLI临时覆盖；缺字段或未知字段失败；BASELINE不被标为最优。

## 13. 验收标准

配置属性测试和快照golden测试通过；effective_config仅引用批准字段；没有API Key字段或真实账户配置。

## 14. 必须运行的命令

```bash
python3.12 -m pytest tests/foundation/config -q
```
```bash
...
- 2026-07-12：配置优先级、FROZEN覆盖拒绝、模式限制和确定性hash通过，验收PASS。
