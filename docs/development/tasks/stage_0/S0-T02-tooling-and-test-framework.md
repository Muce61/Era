...
交付并验证“依赖、格式化、类型检查和测试框架”这一单一能力，使其可独立提交、审查和回滚。
...
- 手册章节：§23 工具链、§27 测试门禁
- rule_id：INVARIANT-ID-GLOBAL-UNIQUE（发布门的工具承载）
- 数据契约：依赖锁、质量命令注册表
...
选择并锁定 Python 3.12、pytest、Hypothesis、ruff、mypy 与构建工具；建立统一质量门脚本。不得选择交易所SDK或业务框架。
...
`pyproject.toml`、依赖锁文件、`scripts/run_quality_gate.py`、测试框架配置
...
第5节能力的最小产物、对应测试、`docs/development/validations/stage_0/S0-T02.md`和精确追踪更新；不得捆绑下一Task。
...
依赖可解析；pytest可收集；格式、类型和依赖一致性命令真实可运行；命令注册表由CI复用。

## 13. 验收标准

五条命令退出码为0；依赖锁可重复安装；质量门不访问网络、Binance或本地密钥。
...
```
```bash
python3.12 scripts/run_quality_gate.py --self-check
```
...
- 2026-07-12：经全Stage授权执行；锁文件与全部质量命令通过，验收PASS。
