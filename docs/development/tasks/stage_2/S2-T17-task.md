...
- dependencies: S2-T10 PASS; S2-T14 PASS; S2-T19 PASS
...
规划并交付“事件聚类”这一单一能力，使其可独立测试、审查和回滚。
...
只实现并验证“事件聚类”及其直接契约、测试和追踪；允许为该单一能力增加最小审计证据。
...
- 数据与能力边界：按同标的/同方向/同setup/context/时间gap/高周期状态形成cluster_id；BTC/ETH和不同setup/context绝不共簇，gap为BASELINE。
- 允许修改路径：`src/era100x/research/stage_2/clustering/`、`tests/research/stage_2/clustering/`，以及本Task validation/TRACEABILITY。禁止修改Stage 1实现/数据、\`docs/spec/**\`和Stage 3+。
- 验证命令：\`uv run python -m pytest tests/research/stage_2/clustering -q\`；\`uv run python scripts/run_quality_gate.py\`。全量研究CLI须由S2-T19冻结后再写入Task新版本，不得当前虚构。
- 验收标准：边界gap、状态段切换、输入排列、跨标的拒绝、稳定cluster ID和全事件覆盖通过。
- 证据模式：\`METHOD_FIXTURE + FULL_CLUSTER_CATALOG\`。无论fixture能力是否可验收，Stage 1最终PASSED与VALID data baseline之前均不得执行本Task。
