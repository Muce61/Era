...
- 验证命令：\`uv run python -m pytest tests/research/stage_2/episodes/reclaim -q\`；\`uv run python scripts/run_quality_gate.py\`。全量研究CLI须由S2-T19冻结后再写入Task新版本，不得当前虚构。
- 验收标准：顺序、超时、边界、缺口、H1/H2参考价分离和参数邻域测试通过。
- 证据模式：\`FIXTURE_CAPABILITY + FULL PARAMETER SURFACE\`。无论fixture能力是否可验收，Stage 1最终PASSED与VALID data baseline之前均不得执行本Task。
