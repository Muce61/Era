...
规划并交付“Reclaim”这一单一能力，使其可独立测试、审查和回滚。
...
只实现并验证“Reclaim”及其直接契约、测试和追踪；允许为该单一能力增加最小审计证据。
...
- 数据与能力边界：仅在有效sweep后判断reference_price >= level+buffer；窗口/buffer来自manifest，不使用历史Ask。
- 允许修改路径：`src/era100x/research/stage_2/episodes/reclaim/`、`tests/research/stage_2/episodes/reclaim/`，以及本Task validation/TRACEABILITY。禁止修改Stage 1实现/数据、\`docs/spec/**\`和Stage 3+。
- 验证命令：\`uv run python -m pytest tests/research/stage_2/episodes/reclaim -q\`；\`uv run python scripts/run_quality_gate.py\`。本Task只做fixture顺序和边界验证；全量候选运行唯一由S2-T10负责。
- 验收标准：顺序、超时、边界、缺口、H1/H2参考价分离和参数邻域测试通过。
- 证据模式：\`FIXTURE_CAPABILITY\`；不得在本Task生成全量参数地形。

## 22. ADR-S2-004预注册绑定

Reclaim fixture必须覆盖[ADR-S2-004](../../decisions/ADR-S2-004-primary-research-definition.md)的T1/T2/T3/T4超时15/30/60/60秒、从`sweep_detection_ts`起算、UTC纳秒、左闭右开及结束边界排除；T2是唯一Primary，其他仅敏感性且不得救援。本Task仍为APPROVED / NOT_EXECUTED。

## 23. ADR-S2-005事件构造绑定

This Task implements ADR-S2-005 reclaim buffers and ADR-S2-004 T1-T4 left-closed/right-open timeouts without reading Hold outcomes.
