...
规划并交付“Sweep Episode”这一单一能力，使其可独立测试、审查和回滚。
...
只实现并验证“Sweep Episode”及其直接契约、测试和追踪；允许为该单一能力增加最小审计证据。
...
- 数据与能力边界：首次从上向下穿越canonical level开始episode，只使用Contract/Trade reference；深度域为预注册RESEARCH。
- 允许修改路径：`src/era100x/research/stage_2/episodes/sweep/`、`tests/research/stage_2/episodes/sweep/`，以及本Task validation/TRACEABILITY。禁止修改Stage 1实现/数据、\`docs/spec/**\`和Stage 3+。
- 验证命令：\`uv run python -m pytest tests/research/stage_2/episodes/sweep -q\`；\`uv run python scripts/run_quality_gate.py\`。全量研究CLI须由S2-T19冻结后再写入Task新版本，不得当前虚构。
- 验收标准：首次穿越、边界相等、缺口、重入、无未来数据和确定性测试通过；不生成EntryIntent。
- 证据模式：\`FIXTURE_CAPABILITY + FULL EVENT FREQUENCY\`。无论fixture能力是否可验收，Stage 1最终PASSED与VALID data baseline之前均不得执行本Task。
