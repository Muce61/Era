...
规划并交付“关键位归一化与仲裁”这一单一能力，使其可独立测试、审查和回滚。
...
只实现并验证“关键位归一化与仲裁”及其直接契约、测试和追踪；允许为该单一能力增加最小审计证据。
...
- 数据与能力边界：按merge tolerance分组、priority_rank、formed_at_ns、稳定hash仲裁；成员ID保留。U-007仍为RESEARCH。
- 允许修改路径：`src/era100x/research/stage_2/key_levels/arbitration/`、`tests/research/stage_2/key_levels/arbitration/`，以及本Task validation/TRACEABILITY。禁止修改Stage 1实现/数据、\`docs/spec/**\`和Stage 3+。
- 验证命令：\`uv run python -m pytest tests/research/stage_2/key_levels/arbitration -q\`；\`uv run python scripts/run_quality_gate.py\`。全量研究CLI须由S2-T19冻结后再写入Task新版本，不得当前虚构。
- 验收标准：排列不变性、tie-break、成员保留、过期/跨标的拒绝通过；参数来自不可变manifest。
- 证据模式：\`FIXTURE_CAPABILITY + FULL U-007 EVIDENCE\`。无论fixture能力是否可验收，Stage 1最终PASSED与VALID data baseline之前均不得执行本Task。
