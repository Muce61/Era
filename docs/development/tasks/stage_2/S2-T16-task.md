...
- dependencies: S2-T10 PASS; S2-T12 PASS; S2-T14 PASS; S2-T19 PASS
...
规划并交付“安慰剂信号”这一单一能力，使其可独立测试、审查和回滚。
...
只实现并验证“安慰剂信号”及其直接契约、测试和追踪；允许为该单一能力增加最小审计证据。
...
- 数据与能力边界：为每个instrument/setup/context生成预注册安慰剂信号，并使用与主事件相同的标签、切分、聚类和报告口径；不得跨setup/context选择最弱安慰剂。
- 允许修改路径：`src/era100x/research/stage_2/baselines/placebo/`、`tests/research/stage_2/baselines/placebo/`，以及本Task validation/TRACEABILITY。禁止修改Stage 1实现/数据、\`docs/spec/**\`和Stage 3+。
- 验证命令：\`uv run python -m pytest tests/research/stage_2/baselines/placebo -q\`；\`uv run python scripts/run_quality_gate.py\`。全量研究CLI须由S2-T19冻结后再写入Task新版本，不得当前虚构。
- 验收标准：安慰剂不泄漏、样本规模/匹配口径一致、随机种子固定、失败安慰剂仍入账；正式结论需全量。
