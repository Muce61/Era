...
- dependencies: S2-T06 PASS; S2-T09 PASS
...
- 数据与能力边界：实现G0-G3研究门：数据、预注册context、结构顺序、1s/5s价格启动；不得包含G4/G5/G6。
- 允许修改路径：`src/era100x/research/stage_2/gates/price/`、`tests/research/stage_2/gates/price/`，以及本Task validation/TRACEABILITY。禁止修改Stage 1实现/数据、\`docs/spec/**\`和Stage 3+。
- 验证命令：\`uv run python -m pytest tests/research/stage_2/gates/price -q\`；\`uv run python scripts/run_quality_gate.py\`。全量研究CLI须由S2-T19冻结后再写入Task新版本，不得当前虚构。
- 验收标准：每个门单独原因、G0缺口硬失败、新结构低点、速度边界、V1_PRICE无G4测试通过。
- 证据模式：\`FIXTURE_CAPABILITY + FULL PASS-RATE EVIDENCE\`。无论fixture能力是否可验收，Stage 1最终PASSED与VALID data baseline之前均不得执行本Task。
