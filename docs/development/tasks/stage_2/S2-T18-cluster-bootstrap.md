...
- dependencies: S2-T15 PASS; S2-T16 PASS; S2-T17 PASS
...
规划并交付“Cluster Bootstrap”这一单一能力，使其可独立测试、审查和回滚。
...
只实现并验证“Cluster Bootstrap”及其直接契约、测试和追踪；允许为该单一能力增加最小审计证据。
...
- 数据与能力边界：按instrument/setup/context以cluster为重采样单位计算预注册指标CI，不以episode伪装独立样本、不池化不同研究setup；固定seed并报告cluster数。
- 允许修改路径：`src/era100x/research/stage_2/statistics/bootstrap/`、`tests/research/stage_2/statistics/bootstrap/`，以及本Task validation/TRACEABILITY。禁止修改Stage 1实现/数据、\`docs/spec/**\`和Stage 3+。
- 验证命令：\`uv run python -m pytest tests/research/stage_2/statistics/bootstrap -q\`；\`uv run python scripts/run_quality_gate.py\`。全量研究CLI须由S2-T19冻结后再写入Task新版本，不得当前虚构。
- 验收标准：seed确定性、cluster权重、空/不足样本、CI覆盖、BTC/ETH独立与episode级错误实现防回归通过。
