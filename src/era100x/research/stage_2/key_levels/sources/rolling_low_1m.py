...
def generate_rolling_lows_1m(bars: list[ContractBar], lineage: SourceLineage) -> list[RawKeyLevel]:
    ordered = sorted(bars, key=lambda bar: bar.bucket_start_ns)
    if any(bar.interval_seconds != INTERVAL_SECONDS for bar in ordered):
        raise ValueError("rolling_low_1m requires closed one-minute bars")
    ...
    for end_index in range(WINDOW - 1, len(ordered)):
        ...
        source_id = stable_id(
            "key-source", "v1", instrument, "rolling_low_1m", source_start, source_end
        )
        raw_id = stable_id(
