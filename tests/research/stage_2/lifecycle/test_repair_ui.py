...
def test_plan_v110_ui_exposes_evidence_driven_observability() -> None:
    ...
    assert 'fetch("/api/v110/status"' in page
    assert "S2P110-T11–T20 Sealed Runtime" in page
    for marker in (
        "progress_percent",
        "processed_units",
        "total_units",
    ...
    assert 'path == "/api/v110/status"' in server
